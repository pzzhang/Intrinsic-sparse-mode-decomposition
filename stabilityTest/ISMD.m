% Given Cov, add pointwise noise
% clear all
% load('data/mediaData.mat')
% noiseN = 8;
noiselevel = 10^(-noiseN);
noise = noiselevel*(2*rand(size(Cov))-1)*c^2*sigma^2/12;
noise = (noise + noise')/2;
Cov = Cov + noise;
...
%% plot the result
ErrorModes = zeros(1,K);
ExactModes = c*sigma*charModes2/sqrt(12);
gISMD = abs(gISMD);
for k = 1 : K
...
%     pause;
%     close(h2);
      errorTemp = repmat(gISMD(:,k),1,K) - ExactModes;
      errorSquare = sum(errorTemp.^2)./sum(ExactModes.^2);
      ErrorModes(1,k) = sqrt(min(errorSquare));
end

% save data
filename3 = ['ErrorModes_e',num2str(noiseN),'.mat'];
save(filename3, 'noiselevel', 'ErrorModes');